...
func sparseMul(sparse *ring.Poly, p *ring.Poly, ctx *ring.Context, w uint64) *ring.Poly {
	positions := make([][]uint64, 0)
	for _, c1 := range sparse.GetCoefficients() {
		buf := make([]uint64, w)
		counter := uint64(0)
		for j, coeff := range c1 {
			if coeff != 0 {
				buf[counter] = uint64(j)
				counter++
			}
		}
		positions = append(positions, buf)
	}
	sparseCoeffs := sparse.GetCoefficients()
	pCoeffs := p.GetCoefficients()
	coeffs := make([][]uint64, len(positions))
	modulus := ctx.Modulus
	for i, ps := range positions {
		vaux := make([]uint64, 2*ctx.N)
		v := make([]uint64, ctx.N)
		for j, pos := range ps {
			sign := sparseCoeffs[i][j] != 1
			for k := uint64(0); k < ctx.N; k++ {
				if sign {
					vaux[pos+k] = addMod(vaux[pos+k], pCoeffs[i][k], modulus[i])
				} else {
					vaux[pos+k] = subMod(vaux[pos+k], pCoeffs[i][k], modulus[i])
				}
			}
		}
		for j := uint64(0); j < ctx.N; j++ {
			v[j] = subMod(vaux[j], vaux[j+ctx.N], modulus[i])
		}
		coeffs[i] = v
	}
	pnew := ctx.NewPoly()
	pnew.SetCoefficients(coeffs)
	return pnew
}
