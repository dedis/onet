...
type SimulationConfig struct {
	...
	ServicePrivateKeys map[network.Address]map[string]kyber.Scalar
...
func LoadSimulationConfig(s, dir, ca string) ([]*SimulationConfig, error) {
	...
	sc := &SimulationConfig{
		...
		ServicePrivateKeys: scf.ServicePrivateKeys,
		TLS:                scf.TLS,
		Config:             scf.Config,
	}
	...
	if ca != "" {
		...
		for _, e := range sc.Roster.List {
			if strings.Contains(e.Address.String(), ca) {
				...
				for name, privkey := range scf.ServicePrivateKeys[e.Address] {
					for i, sid := range e.ServiceIdentities {
						if sid.Name == name {
							e.ServiceIdentities[i] = network.NewServiceIdentity(name, sid.Public, privkey)
						}
					}
				}
...
func (sc *SimulationConfig) Save(dir string) error {
	...
	scf := &SimulationConfigFile{
		...
		ServicePrivateKeys: sc.ServicePrivateKeys,
		TLS:                sc.TLS,
		Config:             sc.Config,
	}
...
func (s *SimulationBFTree) CreateRoster(sc *SimulationConfig, addresses []string, port int) {
	...
	sc.ServicePrivateKeys = make(map[network.Address]map[string]kyber.Scalar)
	for c := 0; c < hosts; c++ {
		...
		sc.ServicePrivateKeys[entities[c].Address] = make(map[string]kyber.Scalar)
		for _, sid := range entities[c].ServiceIdentities {
			sc.ServicePrivateKeys[entities[c].Address][sid.Name] = sid.GetPrivate()
		}
	}
